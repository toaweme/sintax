...
type Sintax interface {
	...
	// Render renders the input string with the given variables
	// it interpolates variables recursively, so it is not safe to use this method for untrusted input
	// like crawler outputs or user-provided strings e.g.
	// some Wikipedia pages contain "{{" and "}}" in their text output
	// and this gets caught by our engine as deep-down variable interpolation
	Render(input string, vars map[string]any) (any, error)
	// RenderSafe is different from Render in that it doesn't interpolate the variables themselves
	// meaning whatever variables are found in input will be inserted from vars, without parsing stings found in vars
	RenderSafe(input string, vars map[string]any) (any, error)
	ExtractDependenciesWithOptions(vars map[string]any, opts *ResolveOptions) ([]string, error)
	ResolveVariablesWithOptions(vars map[string]any, opts *ResolveOptions) (map[string]any, error)
}
```
