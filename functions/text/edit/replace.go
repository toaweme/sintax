...
// ReplacePattern replaces every match of the RE2 pattern in s with replacement,
// which may reference capture groups with $1, $2 and so on. An empty replacement
// deletes each match, and an invalid pattern is an error.
...
// Reverse reverses s by rune, so multi-byte characters stay intact. A cluster
// built from several code points (a combining sequence, or an emoji) is reversed
// code point by code point, which can reorder it.
